...
impl Hgrid {
    ...
    pub fn write(&self, path: &Path) -> std::io::Result<()> {
        ...
        let reversed_nodes: HashMap<u32, (Vec<f64>, Option<Vec<f64>>)> = self
            .nodes
            .hash_map()
            .iter()
...
mod tests {
    ...
    fn test_write_sample_nwatl_hgrid() {
        ...
        let nodes_hash_map: HashMap<u32, (Vec<f64>, Option<Vec<f64>>)> = points
            .iter()
            .enumerate()
        ...
        let start = Instant::now();
        let elements_hash_map: HashMap<u32, Vec<u32>> = triangulation
            .triangles
            .chunks(3)
            .enumerate()
            .map(|(index, triangle)| {
